...
mit 6.824课程提供了[GFS FAQ](https://pdos.csail.mit.edu/6.824/papers/gfs-faq.txt)，本文为简单翻译。

<!--more-->

# GFS相关问题

## 为什么原子性的记录只保证最少一次，而不是精确的一次？
...
## 应用程序如何知道块的哪些部分是padding，哪些是重复的记录
...
## 给定原子记录后，客户如何找到他们的数据，将其以不可预测的偏移量写入文件中？
...
## 如果一个程序使用了标准的POSIX文件操作接口，为了使用GFS，它是否需要进行修改
...
## 假设S1是块的Primary，并且master和S1之间的网络故障。 master会注意到这个问题并指定其他服务器作为Primary，比如S2。 由于S1实际上并没有失败，所以现在在同一个块中存在两个Primary？
> ```
...
> multiple servers, and has more automated master failure recovery.A: That idea simplified initial deployment but was not so great in the long run. This article -- https://queue.acm.org/detail.cfm?id=1594206 -- 
